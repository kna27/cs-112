...
public class FlightPathGraph {
    ...
    // Adjacency list of cities. Each index corresponds to a City node which is the
    // head of a linked list.
    // Each list is an edge list, meaning the first node has an edge to all the rest
    // of the nodes in the list.
    public City[] flightPaths;

    // i.e. if flightPaths[0].city is "New York", and flightPaths[0].next.city is
    // "London", it means New York
    // has a directed edge to London. If New York is also placed in Londons list,
    // the edge is then undirected.

    /*
     * Constructor which initializes the adjacency list with the given verticies,
     * with no edges
     * 
     ...
     * 
     * @param cities String array of cities which are the unique vertices for the
     * graph
     */
     ...
     * 
     * Add a new City node containing the arrival city to the END of the departure
     * citys edgelist
     * Don't insert the edge if it already exists
     * 
     * Note: Insert all edges as directed. Directed edges will appear as a red line
     * in the driver.
     * Undirected edges will appear as black (you can test by inserting the same
     * edge twice, once in reverse. i.e u -> v and then v -> u)
     * 
    ...
    public void addEdge(String departure, String arrival) {
        ...
        for (int i = 0; i < flightPaths.length; i++) {
            if (flightPaths[i].city.equals(departure)) {
                City curr = flightPaths[i];
                while (curr.next != null) {
                    if (curr.next.city.equals(arrival)) {
                        return;
                    }
                    curr = curr.next;
                }
                curr.next = new City(arrival);
                return;
            }
        }
    }
    ...
    public void removeEdge(String departure, String arrival) {
        for (int i = 0; i < flightPaths.length; i++) {
            if (flightPaths[i].city.equals(departure)) {
                City prev = flightPaths[i];
                City curr = prev.next;
                while (curr != null) {
                    if (curr.city.equals(arrival)) {
                        prev.next = curr.next;
                        break;
                    }
                    prev = curr;
